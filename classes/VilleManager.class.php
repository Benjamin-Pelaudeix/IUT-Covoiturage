...
class VilleManager
{
    ...
    public function getAllVilleArriveeWhereExistTrajetFromVille1($id) {
        $listeVille = array();
        $requete = $this->db->prepare('SELECT vil_num2 AS vil_num, vil_nom FROM parcours JOIN ville v on parcours.vil_num2 = v.vil_num WHERE par_num IN (SELECT propose.par_num FROM propose) AND vil_num1 =' . $id);
        $requete->execute();
        while ($ville = $requete->fetch(PDO::FETCH_ASSOC))
            $listeVille[] = new Ville($ville);
        return $listeVille;
